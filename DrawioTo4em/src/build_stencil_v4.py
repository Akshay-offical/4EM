...
def make_userobj_xml_ex(label, em_type, style, w, h, extra_attrs=None):
    """
    Same as make_userobj_xml, but can bake in one extra custom attribute
    beyond em_type — used for sub-type variant shapes (e.g. a Problem
    shape whose "problem_type" is pre-set to "Weakness"). draw.io keeps
    this attached to the shape permanently, same as em_type; the user can
    still see/edit it via Edit > Edit Data if they want to double check it,
    but doesn't need to for the value to be correct.
    """
    gm = ET.Element("mxGraphModel")
    root = ET.SubElement(gm, "root")
    ET.SubElement(root, "mxCell", attrib={"id": "0"})
    ET.SubElement(root, "mxCell", attrib={"id": "1", "parent": "0"})
    attrib = {"label": label, "em_type": em_type, "id": "2"}
    if extra_attrs:
        attrib.update(extra_attrs)
    obj = ET.SubElement(root, "UserObject", attrib)
    inner = ET.SubElement(obj, "mxCell", {
        "style": style, "vertex": "1", "parent": "1",
    })
    ET.SubElement(inner, "mxGeometry", {
...
def node_with_subtype(title, em_type, style, subtype_attr, subtype_value,
                       w=180, h=60, name=None):
    """
    A node shape that pre-selects one of several 4EM sub-type options
    (e.g. Problem's Type: Problem/Weakness/Threat) by baking a custom
    attribute directly into the shape. Each option gets its own draggable
    stencil entry — same pattern already used for AND/OR/PartOF variants —
    so picking the right one is just dragging the right shape, no dialog
    or hidden text convention to learn.
    """
    label = name if name else "<name>"
    return {
        "xml": make_userobj_xml_ex(label, em_type, style, w, h,
                                    extra_attrs={subtype_attr: subtype_value}),
...
C = {
    ...
    "Feature": "shape=trapezoid;" "perimeter=trapezoidPerimeter;" "whiteSpace=wrap;" "html=1;" "fillColor=#A9CCE3;" "strokeColor=#000000;" "strokeWidth=2;",
...
goal_shapes = [
    ...
    node("KPI",         "KPI",         C["KPI"]),
    node_with_subtype("Problem",  "Problem", C["Problem"], "problem_type", "Problem"),
    node_with_subtype("Weakness", "Problem", C["Problem"], "problem_type", "Weakness"),
    node_with_subtype("Threat",   "Problem", C["Problem"], "problem_type", "Threat"),
...
write_stencil("generated_stencils/4EM_GoalModel_Stencil_v4.xml", goal_shapes)
...
write_stencil("generated_stencils/4EM_BusinessProcess_Stencil_v4.xml", bp_shapes)
...
write_stencil("generated_stencils/4EM_ActorsResources_Stencil_v4.xml", ar_shapes)
...
write_stencil("generated_stencils/4EM_Concepts_Stencil_v4.xml", concepts_shapes)

# ══════════════════════════════════════════════════════════════════════════════
# 5. PRODUCT-SERVICE MODEL
# ══════════════════════════════════════════════════════════════════════════════
ps_shapes = [
    node_with_subtype("Unspecific", "Unspecific/Product/Service", C["UnspecPS"], "specification", "Unspecific", w=180, h=55),
    node_with_subtype("Product",    "Unspecific/Product/Service", C["UnspecPS"], "specification", "Product",    w=180, h=55),
    node_with_subtype("Service",    "Unspecific/Product/Service", C["UnspecPS"], "specification", "Service",    w=180, h=55),
...
write_stencil("generated_stencils/4EM_ProductService_Stencil_v4.xml", ps_shapes)

# ══════════════════════════════════════════════════════════════════════════════
# 6. TECHNICAL COMPONENTS AND REQUIREMENTS MODEL
# ══════════════════════════════════════════════════════════════════════════════
tech_shapes = [
    node_with_subtype("IS Requirement (Functional)",    "IS Requirement", C["ISReq"], "req_type", "Functional",    w=120, h=60),
    node_with_subtype("IS Requirement (Nonfunctional)", "IS Requirement", C["ISReq"], "req_type", "Nonfunctional", w=120, h=60),
...
write_stencil("generated_stencils/4EM_Technical_Stencil_v4.xml", tech_shapes)

# ══════════════════════════════════════════════════════════════════════════════
# 7. BUSINESS RULE MODEL — Goal/Process/Constraint REMOVED
# ══════════════════════════════════════════════════════════════════════════════
rule_shapes = [
    node_with_subtype("Rule (Derivation)",           "Rule", C["Rule"], "rule_type", "Derivation Rule"),
    node_with_subtype("Rule (Event-action)",         "Rule", C["Rule"], "rule_type", "Event-action Rule"),
    node_with_subtype("Rule (Static Constraint)",    "Rule", C["Rule"], "rule_type", "Static Constraint Rule"),
    node_with_subtype("Rule (Transition Constraint)","Rule", C["Rule"], "rule_type", "Transition Constraint Rule"),
    bare_node("AND",    "AND",    C["AND"],   w=60, h=60),
    bare_node("OR",     "OR",     C["OR"],    w=60, h=60),
    bare_node("AND/OR", "AND/OR", C["ANDOR"], w=60, h=60),
    *helper_nodes(),
...
write_stencil("generated_stencils/4EM_BusinessRule_Stencil_v4.xml", rule_shapes)
